...
void G_Say( gentity_t *ent, gentity_t *target, int mode, const char *chatText ) {
	...
	case SAY_TEAM:
		...
		{
			Com_sprintf (name, sizeof(name), EC"(%s%s%c%c"EC")"EC": ", 
				NM_SerializeUIntToColor(ent - g_entities), ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE );
			locMsg = location;
		}
		else
		{
	...
	case SAY_TELL:
		...
		Com_sprintf (name, sizeof(name), EC"[%s%s%c%c"EC"]"EC": ", NM_SerializeUIntToColor(ent - g_entities), ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE );
...
static void Cmd_VoiceCommand_f(gentity_t *ent)
{
	...
	for (n = 0; n < level.maxclients; n++) {
		...
		if (!(g_gametype.integer == GT_CTF || g_gametype.integer == GT_CTY) && level.clients[n].sess.sessionTeam != OtherTeam(ent->client->sess.sessionTeam)) {
			gentity_t *loc = Team_GetLocation(ent);
			if (loc)
				te->s.powerups = loc->health;
		}
#endif
...
void Cmd_ServerStatus2_f(gentity_t *ent)
{
	...
	Com_sprintf(string, sizeof(string), "g_enhancedLocations");
	ServerCfgColor(string, g_enhancedLocations.integer, ent);
